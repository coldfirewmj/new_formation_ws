...
namespace ego_planner
{
  ...
  bool EGOPlannerManager::followcarReplan(
      const Eigen::Vector3d &start_pt, const Eigen::Vector3d &start_vel,
      const Eigen::Vector3d &start_acc, const Eigen::Vector3d &local_target_pt,
      const Eigen::Vector3d &local_target_vel, std::vector<Eigen::Vector3d> &innerpts,
      std::vector<double> &taj_durations, const bool touch_goal)
  {
    //第一步 初始化
      ...
      poly_traj::MinJerkOpt &initMJO)
  {
    ...
    else /*** case 2: initialize from previous optimal trajectory ***/
    {
      ...
      initMJO.reset(headState, tailState, piece_nums);
      std::cout<<"innerPs is: "<<innerPs.cols()<<" and piece_dur_vec is: "<<piece_dur_vec.size()<<std::endl;
      initMJO.generate(innerPs, piece_dur_vec);
    }
