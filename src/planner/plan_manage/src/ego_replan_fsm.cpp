...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    formation_pos_.resize(3, formation_num);
    std::vector<double> pos;
    for (int i = 1; i < formation_num; i++)
    {
      nh.getParam("formation/drone" + to_string(i), pos);
      formation_pos_.col(i) << pos[0], pos[1], pos[2];
    }
    drone_formation_pos_ = formation_pos_.col(planner_manager_->pp_.drone_id);
    ...
    {
      waypoint_sub_ = nh.subscribe("/swarm/path", 1, &EGOReplanFSM::followtrajCallback, this);
    }
    else
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
               ...
               (final_goal_ - pos).norm() < no_replan_thresh_) // case 2: assign the next waypoint
      {
        ...
        wpt_id_++;
        Eigen::Vector3d headto = (wps_[wpt_id_] - formation_start_).normalized();
        headto << 1, 0, 0;
        //以下为匈牙利算法部分，用于在大角度机动时，由于飞行方向变化所需要进行的的编队变换分配计算
        //方法是计算包括自己在内的其他无人机位置，然后利用匈牙利算法计算变换之后应该分配到的位置，并更新自身位置
        //由于是分布式计算，可能会出现两个无人机分配到一个位置？
        pos_map.insert(pair<int, Eigen::Vector3d>(planner_manager_->pp_.drone_id, pos));
        for (size_t i = 0; i < planner_manager_->traj_.swarm_traj.size(); i++)
        {
          int swarmid = planner_manager_->traj_.swarm_traj[i].drone_id;
          if (swarmid < 1)
          {
            continue;
          }
          double t_now = ros::Time::now().toSec() - planner_manager_->traj_.swarm_traj[i].start_time;
          poly_traj::Trajectory trajectory = planner_manager_->traj_.swarm_traj[i].traj;
          Eigen::Vector3d swarm_pos = trajectory.getPos(t_now);
          pos_map.insert(pair<int, Eigen::Vector3d>(swarmid, swarm_pos));
        }
        ...
        for (auto iter = pos_map.begin(); iter != pos_map.end(); iter++)
        {
          ...
          auto now_pos = iter->second;
          for (int i = 1; i < formation_pos_.cols(); i++)
          {
            Eigen::Vector3d tar_p;
            tar_p.x() = (headto(0) * (formation_pos_(0, i)) - headto(1) * formation_pos_(1, i)) + formation_start_(0);
            tar_p.y() = (headto(1) * (formation_pos_(0, i)) + headto(0) * formation_pos_(1, i)) + formation_start_(1);
            tar_p.z() = formation_pos_(2, i) + formation_start_(2);
            distance_cost.push_back((now_pos - tar_p).norm());
          }
          DistMatrix.push_back(distance_cost);
          distance_cost.clear();
        }
        double total_cost = hun_.hungarian_Solve(DistMatrix, Assignment);
        int new_formation;
        new_formation = Assignment[planner_manager_->pp_.drone_id - 1] + 1;
        cout << "id is: " << planner_manager_->pp_.drone_id << " formation is:" << new_formation << endl;
        drone_formation_pos_ = formation_pos_.col(new_formation);
        planner_manager_->set_formation(Assignment);
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    // cout << "[" + pos_call + "]"<< "Drone:" << planner_manager_->pp_.drone_id << ", from " + state_str[pre_s] + " to " + state_str[int(new_state)] << endl;
  }
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    if (plan_success)
    {
      ...
      poly_traj_pub_.publish(poly_msg);
      broadcast_ploytraj_pub_.publish(MINCO_msg);
      // std::cout << "Size of MINCO_msg: " << sizeof(MINCO_msg) << " bytes" << std::endl;
    }
  ...
  {
    Eigen::Vector3d headto = (next_wp - formation_start_).normalized();
    headto << 1, 0, 0;
    final_goal_ = next_wp + Eigen::Vector3d(headto(0) * drone_formation_pos_(0) - headto(1) * drone_formation_pos_(1),
                                            headto(1) * drone_formation_pos_(0) + headto(0) * drone_formation_pos_(1),
                                            drone_formation_pos_(2));
    bool success = false;
  ...
  void EGOReplanFSM::waypointCallback(const quadrotor_msgs::GoalSetPtr &msg)
  {
    ...
    if (wps_.size() > 0)
    {
      formation_start_ = wps_[wps_.size() - 1];
    }
    Eigen::Vector3d end_wp(msg->goal[0], msg->goal[1], msg->goal[2]);
    wps_.push_back(end_wp);
    wpt_id_ = wps_.size() - 1;
  ...
  void EGOReplanFSM::followtrajCallback(const car_msgs::TrajectoryPtr &msg)
  {
    ROS_INFO("recv point,drone_ID %d", planner_manager_->pp_.drone_id);
    if (!have_target_)
    {
    ...
    std::vector<double> taj_durations;
    for (auto i=0;i<msg->trajectorypoints.size();i++)
    {
      Eigen::Vector3d carpos, targetpos, targetvel, targetdir;
      carpos(0)=msg->trajectorypoints[i].position.x;
      carpos(1)=msg->trajectorypoints[i].position.y;
      carpos(2)=1;
      double carvel = msg->trajectorypoints[i].velocity;
      //根据yaw角计算位置(未来可能需要根据实际值进行调整)
      double caryaw = msg->trajectorypoints[i].yaw;
      targetdir << cos(caryaw),sin(caryaw),0;
      targetpos = carpos + Eigen::Vector3d(
        targetdir(0) * drone_formation_pos_(0) - targetdir(1) * drone_formation_pos_(1),
        targetdir(1) * drone_formation_pos_(0) + targetdir(0) * drone_formation_pos_(1),
        drone_formation_pos_(2));
      //未来可能加入速度
      if (i==(msg->trajectorypoints.size()-1))
      {
        touch_goal_=true;
      }
      if ((odom_pos_ - targetpos).norm() > planning_horizen_)
      {
        local_target_pt_ = targetpos;
        targetvel << carvel*cos(caryaw), carvel*sin(caryaw), 0;
        local_target_vel_ = targetvel;
        break;
      }
      traj_pts.push_back(targetpos);
    }
    //需要在planmanager里面写一个根据轨迹进行编队的的新函数,由于需要跟车，因此直接输入时间和点进行优化
    planner_manager_->followcarReplan(start_pt_, start_vel_, start_acc_,
        local_target_pt_, local_target_vel_, traj_pts, taj_durations, touch_goal_);
  }
