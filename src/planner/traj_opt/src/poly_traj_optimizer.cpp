...
bool PolyTrajOptimizer::formationGradCostP(const int i_dp,
                                             ...
                                             double &costp)
  {
    ...
    for (int id = 0; id < swarm_trajs_->size(); ++id)
    {
      ...
      if (std::abs(swarm_trajs_->at(id).drone_id-drone_id_)>10)
      {
        // std::cout<<"the other_id_ is:"<<swarm_trajs_->at(id).drone_id<<std::endl;
        // std::cout<<"the drone_id_ is:"<<drone_id_<<std::endl;
        // std::cout<<"the id gap is:"<<std::abs(swarm_trajs_->at(id).drone_id-drone_id_)<<std::endl;
        continue;
      }
      count_drone+=1;
      double traj_i_satrt_time = swarm_trajs_->at(id).start_time;
      ...
      l += (swarm_p - O).dot(a) - formation_.col(drone_formation_id_[swarm_trajs_->at(id).drone_id-1]+1).dot(a);
      dl_dt += a.dot(swarm_v);
    }
    if (count_drone>0)
    {
      ...
      tar_p.x() = (headto(0) * formation_(0, drone_formation_id_[drone_id_-1]+1) + a(0) * l - 
      headto(1) * formation_(1, drone_formation_id_[drone_id_-1]+1)) + O(0);
      tar_p.y() = (headto(1) * formation_(0, drone_formation_id_[drone_id_-1]+1) + a(1) * l + 
      headto(0) * formation_(1, drone_formation_id_[drone_id_-1]+1)) + O(1);
      tar_p.z() = a(2) * l + formation_(2, drone_formation_id_[drone_id_-1]+1) + O(2);
  ...
  void PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    for (int i = 1; i < formation_num_; i++)
    {
      ...
      drone_formation_id_.push_back(i-1);
    }
  }
